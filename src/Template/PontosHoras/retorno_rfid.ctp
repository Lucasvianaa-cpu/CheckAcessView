...
<div class="container d-flex align-items-center" style="height: 100vh;">
  <div class="card">
    <div>
      <?php if (!empty($funcionario->user->caminho_foto)) : ?>
        <?= $this->Html->image($funcionario->user->caminho_foto, ['style' => 'min-height: 80px; max-height: 80px; border-radius: 40px;']); ?>

      <?php else : ?>
        <?= $this->Html->image('perfil.png', ['style' => 'min-height: 80px; max-height: 80px; border-radius: 40px;']); ?>
      <?php endif; ?>
    </div>
    <div style="margin-left: 20px;">
      <label>Tag RFID:</label>
      <span style="font-size: 18px; margin-left: 6px; color: #FFFFFF;"><?= $funcionario->user->uid_rfid ?></span>
      <label>Funcionário:</label>
      <span style="font-size: 18px; margin-left: 6px; color: #FFFFFF;"><?= $funcionario->user->nome ?></span>
      <span style="font-size: 18px; color: #FFFFFF;"><?= $funcionario->user->sobrenome ?></span>
    </div>
  </div>
  <div class="receipt" style="margin-top: -80px; background-color: #FFFFFF; box-shadow: 0 20px 27px 0 rgba(0, 0, 0, 0.05);">
    <div class="ponto-registrado">
      <?= $this->Html->image('../img/logo/correto.png', ['style' => 'min-height: 24px; max-height: 24px;']); ?>
      <h3 class="font-weight-black text-dark" style="margin-left: 10px;">Ponto registrado</h3>
    </div>
    <div class="ponto-registrado">
      <?= $this->Html->image('../img/logo/calendario.png', ['style' => 'min-height: 24px; max-height: 24px;']); ?>
      <h3 class="font-weight-black text-dark" style="margin-left: 10px;">Dia: </h3>
      <span style="font-size: 18px; margin-left: 6px;">
        <?= date('d/m/Y', strtotime($pontosHora->data_ponto)) ?>
      </span>
